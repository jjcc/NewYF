#file 1/3 from https://medium.com/google-cloud/build-your-own-ai-powered-stock-analyzer-for-free-5d26248c5173
import pandas as pd
import json
...
from new_yf import get_data_by_list
...
class StockAPI:
    ...
    def get_stock_info_yf(self,stock):
        today = datetime.today().strftime('%Y-%m-%d')
        threemonths_ago = datetime.today() - pd.DateOffset(months=3)
        threemonths_ago = threemonths_ago.strftime('%Y-%m-%d')
        start_date = threemonths_ago
        end_date = today
        data = get_data_by_list([stock], start_date, end_date)
        return data[stock]
